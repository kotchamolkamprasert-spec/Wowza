...
def _shutdown():
    """Close the serial ports, then stop serving."""
    time.sleep(0.2)
    if hub:
        hub.stop()
    time.sleep(0.4)          # let the reader threads notice and close
    if server:
        server.shutdown()
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path.startswith("/api/shutdown"):
            # A graceful stop matters more than it sounds.  An ESP32 accepts one
            # Bluetooth client at a time, and a server killed outright never
            # closes the port - the board then believes the session is still
            # open and refuses to reconnect until somebody resets it by hand.
            self._json({"ok": True})
