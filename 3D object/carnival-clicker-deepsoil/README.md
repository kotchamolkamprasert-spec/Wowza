...
Three things keep that from looking steppy:

- The model **eases** toward the click count rather than snapping to it, so
  growth flows on between clicks instead of twitching on each one.
- Each stage **swells slightly across its own span** before handing over, so
  every click moves something, not just the ones that trigger a swap.
- A stage change is a **growth animation, not a cross-fade**. Each stage is
  split into `terrain` and `flora` when it loads, and the new planting rises
  out of the ground with an overshooting ease while the old one retires. The
  incoming trees stay fully opaque the whole way up - fading them in as well
  just made them look like ghosts.

The land underneath does cross-fade, but consecutive stages share most of it so
it is close to invisible. The one rule there: the new ground reaches full
opacity *before* the old ground starts to fade. If both are half transparent at
once the black background shows through and the whole island dims mid-swap.
...
That fallback earned its keep: the later stages contain a **camel**
(`camel_body`, `camel_hump`, `camel_neck`, `camel_leg`, `camel_head`) and a
**bird**, neither of which appeared in the earlier files. They showed up grey
until they were given colours.
...
**Stop it with `stop.bat` or `Ctrl+C`, not by killing the window.** An ESP32
accepts one Bluetooth client at a time, and a server killed mid-connection never
closes the port, so the board believes that session is still open and refuses to
...
`stop.bat`, `play.bat` and `run_kiosk.bat` all go through `_clear.bat`, which
asks the running server to shut down properly via `POST /api/shutdown` before
resorting to killing it. That endpoint closes the serial ports first, which is
the whole point: it is what stops a board being left in that stuck state.
